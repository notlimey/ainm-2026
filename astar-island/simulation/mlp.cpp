               ...
               std::vector<std::vector<int>>& grid, int& W, int& H)
{
    ...
    for(uint32_t i = 0; i < count; i++)
    {
        ...
        f.read((char*)&round, 4); f.read((char*)&seed, 4);
        ...
        else f.seekg((int64_t)w * h * 4, std::ios::cur);
    }
                       ...
                       std::vector<std::vector<std::vector<float>>>& gt, int& W, int& H)
{
    ...
    for(uint32_t i = 0; i < count; i++)
    {
        ...
        else f.seekg((int64_t)w * h * NUM_CLASSES * 4, std::ios::cur);
    }
    return false;
}
                      ...
                      const std::vector<std::vector<std::vector<float>>>& pred)
{
    ...
    f.write((char*)&round, 4); f.write((char*)&seed, 4);
